...
export function formatBaseSkillFrontmatter(template: SkillTemplate, version: string): string[] {
  const lines = [
    ...
    `compatibility: ${template.compatibility || "Requires spec CLI."}`,
    "metadata:",
    `  author: ${template.metadata?.author || "spec"}`,
...
export function formatManifestContent(skills: SkillTemplate[], version: string): string {
  const lines = [
    ...
    "# Skills Index",
    "",
    "This file is auto-generated by spec. Agents can read it at session start",
