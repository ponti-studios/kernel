...
describe('updateConfig', () => {
  ...
  it('preserves untouched fields', async () => {
    await createDefaultConfig(tmpDir, { tools: ['opencode'], delivery: 'commands' });
    const updated = await updateConfig(tmpDir, { profile: 'extended' });
    expect(updated.tools).toEqual(['opencode']);
    expect(updated.delivery).toBe('commands');
  });
});
...
describe('ConfigSchema', () => {
  ...
  it('rejects invalid profile', () => {
    expect(() =>
      ConfigSchema.parse({ tools: ['opencode'], profile: 'invalid' })
    ).toThrow();
  });

  it('rejects invalid delivery', () => {
    expect(() =>
      ConfigSchema.parse({ tools: ['opencode'], delivery: 'invalid' })
    ).toThrow();
  });
});
