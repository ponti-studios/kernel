...
export function createClaudeCodeHooksHook(
  ...
  return {
    ...
    "chat.message": async (
      ...
      if (!isHookDisabled(config, "UserPromptSubmit")) {
        ...
        if (result.messages.length > 0) {
          ...
          log(`[grid-claude-code-hooks] Injecting ${result.messages.length} hook messages`, {
            sessionID: input.sessionID,
    ...
    "tool.execute.after": async (
      ...
      // This ensures plugin tools (call_grid_agent, delegate_task, task) that return strings
