#!/usr/bin/env python3
"""
Set up the central tutorials repository for ghostwire coding tutor.

Usage:
    python setup_tutorials.py
    python setup_tutorials.py --create-github-repo

Creates ~/ghostwire-tutorials/ if it doesn't exist, initializes git,
...
def get_tutorials_repo_path():
    """Get the path for the tutorials repo (~/ghostwire-tutorials/)."""
    return Path.home() / "ghostwire-tutorials"


README_CONTENT = """# Coding Tutor - My Learning Journey

This repository contains my personalized coding tutorials created with the [ghostwire](https://github.com/EveryInc/ghostwire) plugin.
    ...
    try:
        ...
        if create_github:
            result = subprocess.run(
                ['gh', 'repo', 'create', 'ghostwire-tutorials', '--private', '--source=.', '--push'],
...
def main():
    parser = argparse.ArgumentParser(
        description="Set up the central tutorials repository for ghostwire coding tutor"
    )
    parser.add_argument(
        "--create-github-repo",
        action="store_true",
