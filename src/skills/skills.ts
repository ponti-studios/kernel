  ...
  try {
    ...
    if (!validation.success) {
      const errs = validation.error.issues
        ...
        .join("; ");
      console.error(`Skill ${dirName} is invalid: ${errs}`);
      return null;
    }
...
export function mergeScopedSkillsWithBuiltins(scopedSkills: ReadonlyArray<Skill>): Skill[] {
  return mergeBuiltinManifestAfterScoped(scopedSkills);
}
