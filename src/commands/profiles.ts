...
import { AGENT_PROMPTS } from "../agents/prompts/index.ts";
...
export const COMMAND_PROFILE_REGISTRY: Record<string, CommandProfile> = {
  ...
  // Primary orchestration controllers
  do: {
    profile_id: "do",
    intent: "Execute and coordinate subagent workflows based on user goals.",
    required_tools: ["delegate_task", "task", "search", "read"],
    runtime_route: "do",
    prompt_append: "Delegate to research, plan, and executor agents as needed.",
    acceptance_checks: ["subagents invoked", "results aggregated", "goal satisfied"],
    default_command: "ghostwire:workflows:execute",
  },
  research: {
    profile_id: "research",
    intent: "Coordinate investigative tasks across multiple research subagents.",
    required_tools: ["delegate_task", "search", "web", "read"],
    runtime_route: "research",
    prompt_append: "Dispatch codebase/world/git researchers and synthesize findings.",
    acceptance_checks: ["context gathered", "sources cited", "insights delivered"],
    default_command: "ghostwire:workflows:plan",
  },
  plan: {
    profile_id: "plan",
    intent: "Produce work plans by interviewing users and analyzing context.",
    required_tools: ["read", "search", "delegate_task"],
    runtime_route: "do",
    prompt_append: "Break goals into deterministic task graphs with checks.",
