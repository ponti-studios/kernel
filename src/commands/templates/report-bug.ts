export const REPORT_BUG_TEMPLATE = `
...
\`\`\`bash
# Get plugin version
cat ~/.claude/plugins/installed_plugins.json 2>/dev/null | grep -A5 "ghostwire" | head -10 || echo "Plugin info not found"
...
\`\`\`bash
gh issue create \\
  --repo EveryInc/ghostwire-plugin \\
  --title "[ghostwire] Bug: [Brief description]" \\
...
Issue: https://github.com/EveryInc/ghostwire-plugin/issues/[NUMBER]
Title: [ghostwire] Bug: [description]
Thank you for helping improve the ghostwire plugin!
