export const REPRODUCE_BUG_TEMPLATE = `
...
\`\`\`
mcp__plugin_ghostwire_pw__browser_navigate({ url: "http://localhost:3000" })
mcp__plugin_ghostwire_pw__browser_snapshot({})
\`\`\`
If server not running, inform user to start \`bin/dev\`.
### Step 2: Navigate to Affected Area
Based on the issue description, navigate to the relevant page:
\`\`\`
mcp__plugin_ghostwire_pw__browser_navigate({ url: "http://localhost:3000/[affected_route]" })
mcp__plugin_ghostwire_pw__browser_snapshot({})
\`\`\`
### Step 3: Capture Screenshots
Take screenshots at each step of reproducing the bug:
\`\`\`
mcp__plugin_ghostwire_pw__browser_take_screenshot({ filename: "bug-[issue]-step-1.png" })
\`\`\`
...
3. **Check for console errors:**
   \`\`\`
   mcp__plugin_ghostwire_pw__browser_console_messages({ level: "error" })
   \`\`\`
...
\`\`\`
mcp__plugin_ghostwire_pw__browser_take_screenshot({ filename: "bug-[issue]-reproduced.png" })
\`\`\`
