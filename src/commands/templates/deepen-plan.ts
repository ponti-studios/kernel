export const DEEPEN_PLAN_TEMPLATE = `
...
ls ~/.claude/skills/
# 3. ghostwire plugin skills
ls ~/.claude/plugins/cache/*/ghostwire/*/skills/
# 4. ALL other installed plugins - check every plugin for skills
find ~/.claude/plugins/cache -type d -name "skills" 2>/dev/null
# 5. Also check installed_plugins.json for all plugin locations
cat ~/.claude/plugins/installed_plugins.json
\`\`\`
**Important:** Check EVERY source. Don't assume ghostwire is the only plugin. Use skills from ANY installed plugin that's relevant.
...
<thinking>
For each major section in the plan, spawn dedicated sub-agents to research improvements. Use the Scout Recon agent type for open-ended research.
</thinking>
**For each identified section, launch parallel research:**
\`\`\`
Task Scout Recon: "Research best practices, patterns, and real-world examples for: [section topic].
Find:
...
mcp__plugin_ghostwire_context7__resolve-library-id: Find library ID for [framework]
mcp__plugin_ghostwire_context7__query-docs: Query documentation for specific patterns
# 3. ghostwire plugin agents (all subdirectories)
find ~/.claude/plugins/cache/*/ghostwire/*/agents -name "*.md" 2>/dev/null
- ghostwire plugin (but SKIP workflow/ agents - only use review/, research/, design/, docs/)
**For ghostwire plugin specifically:**
