...
import type { SkillMcpClientInfo, SkillMcpServerContext } from "../../skill-mcp-manager/types";
...
export function createSkillTool(options: SkillLoadOptions = {}): ToolDefinition {
  ...
  return tool({
    ...
    args: {
      name: tool.schema
        .string()
        .describe("The skill identifier from available_skills (e.g., 'code-review')"),
    },
    async execute(args: SkillArgs, ctx?: { agent?: string }) {
      ...
      const output = [`## Skill: ${skill.name}`, "", `**Base directory**: ${dir}`, "", body];
