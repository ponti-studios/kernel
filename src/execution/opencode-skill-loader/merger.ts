...
export function resolveDeterministicSkillsFirstWins(
  orderedSkills: LoadedSkill[],
): DeterministicResolutionResult {
  const seenSkills = new Map<string, LoadedSkill>();
  const resolvedSkills: LoadedSkill[] = [];
  const collisions: DeterministicResolutionResult["collisions"] = [];

  for (const skill of orderedSkills) {
    const existing = seenSkills.get(skill.name);
    if (!existing) {
...
$ARGUMENTS
  ...
  const allowedTools =
    ...
    (fileMetadata["allowed-tools"]
      ? parseAllowedToolsFromMetadata(fileMetadata["allowed-tools"])
      : undefined);
...
/**
 * Merge skills from multiple sources with deterministic precedence.
 *
 * Priority order (lowest to highest):
 * 1. Built-in skills (bundled with plugin)
 * 2. Config-declared skills (from ghostwire.json)
...
export function mergeSkills(
  ...
  // 3. Apply scoped skills (highest priority - wins over built-in and config)
  ...
  // 4. Apply config overrides (disable/enable lists)
  for (const [name, entry] of Object.entries(normalizedConfig.entries)) {
    if (entry === true) continue;
    if (entry === false) {
