...
def get_tutorials_directory():
    """Get the tutorials directory (~/ghostwire-tutorials/)."""
    return Path.home() / "ghostwire-tutorials"
...
def index_tutorials(tutorials_dir=None):
    ...
    Args:
        tutorials_dir: Path to tutorials directory (defaults to ~/ghostwire-tutorials/)
...
def format_human_readable(tutorials):
    ...
    if not tutorials:
        return "No tutorials found. Check if ~/ghostwire-tutorials/learner_profile.md exists - if not, onboard the learner first. If it exists, create their first tutorial using their profile context."
...
def main():
    ...
    parser.add_argument(
        "--tutorials-dir",
        help="Path to tutorials directory (defaults to ~/ghostwire-tutorials/)",
        default=None
    )
    parser.add_argument(
        "--format",
    ...
    try:
        ...
        if args.format == "json":
            if not tutorials:
                print(json.dumps({
                    "tutorials": [],
                    "message": "No tutorials found. Check if ~/ghostwire-tutorials/learner_profile.md exists - if not, onboard the learner first. If it exists, create their first tutorial using their profile context."
                }, indent=2))
            else:
