export const RELEASE_DOCS_TEMPLATE = `
...
# Count skills
ls -d plugin/skills/*/ 2>/dev/null | wc -l
# Count MCP servers
ls -d plugin/mcp-servers/*/ 2>/dev/null | wc -l
\`\`\`
...
# Verify counts match
echo "Agents in files: $(ls plugin/agents/*.md | wc -l)"
grep -o "[0-9]* specialized agents" plugin/docs/index.html
echo "Commands in files: $(ls plugin/commands/*.md | wc -l)"
grep -o "[0-9]* slash commands" plugin/docs/index.html
\`\`\`
