...
import type { McpOAuthProviderOptions } from "../mcp-oauth/provider";
...
type OAuthProviderLike = Pick<McpOAuthProvider, "tokens" | "login">;
type OAuthProviderFactory = (options: McpOAuthProviderOptions) => OAuthProviderLike;
...
export class SkillMcpManager {
  ...
  private authProviders: Map<string, OAuthProviderLike> = new Map();
  private cleanupRegistered = false;
  private cleanupInterval: ReturnType<typeof setInterval> | null = null;
  private readonly IDLE_TIMEOUT = 5 * 60 * 1000;
  private readonly createOAuthProvider: OAuthProviderFactory;

  constructor(options?: { createOAuthProvider?: OAuthProviderFactory }) {
    this.createOAuthProvider =
      options?.createOAuthProvider ?? ((providerOptions) => new McpOAuthProvider(providerOptions));
  }
  ...
  private getOrCreateAuthProvider(
    ...
    const provider = this.createOAuthProvider({
      serverUrl,
  ...
  private async createHttpClient(
    ...
    // we only need the provider inside the block where oauth config
    // is present, and the factory returns an OAuthProviderLike rather than a
    // full McpOAuthProvider.  narrow the type accordingly and avoid the
    // `possibly undefined` checks by keeping the variable scoped.
    if (config.oauth) {
      const authProvider = this.getOrCreateAuthProvider(config.url, config.oauth);
      let tokenData = authProvider.tokens();

      const isExpired =
        tokenData?.expiresAt != null && tokenData.expiresAt < Math.floor(Date.now() / 1000);
  ...
  private async handleStepUpIfNeeded(error: Error, config: ClaudeCodeMcpServer): Promise<boolean> {
