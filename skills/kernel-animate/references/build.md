...
A construction skill. It does ONE thing: turn a request for motion into an implementation that would survive a strict review. It does not audit a codebase (that's the improve workflow), critique a diff (that's the review workflow), or hunt for places that could animate (that's the opportunities workflow).

## Operating Posture

You are a senior design engineer building the animation yourself. The bar is Emil Kowalski's animation philosophy — the same bar the review workflow enforces. Write it so it passes that review the first time.
...
If the task needs a *component* rather than an animation — a toast, a drawer, a command menu, a dropdown — use a battle-tested library (Sonner, Vaul, cmdk, a Radix/Base UI primitive) instead of hand-rolling. Hand-rolling those is how you end up with a `<div>` dropdown and no focus management.
...
Self-check before you finish. Each of these is an automatic block in the review workflow:
