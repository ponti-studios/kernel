...
- `kernel-dev-typescript` for type-system issues
- `kernel-dev-testing` for test design, flaky tests, or coverage questions
- `kernel-dev-react` / `kernel-dev-react-native` for UI-layer failures
- `kernel-dev-database` for migration or schema failures
