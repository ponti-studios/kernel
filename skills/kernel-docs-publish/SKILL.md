...
It does not replace the craft skills used to make the artifacts. Use `write-video` for script and production-plan work. Use the appropriate slides/deck workflow when slides are required.
...
3. If the video script or production plan does not exist, route that work to `write-video`.
